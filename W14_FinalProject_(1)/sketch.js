...
function setup() {
  ...
  for (let i = 0; i < 300; i++) {
    vehicles.push(new Vehicle(random(0, width/2), height, "white"));
    //vehicles.push(new Vehicle(0, random(0, windowHeight), "#E0E0E0"));
    vehicles.push(new Vehicle(random(width/2, width), height, "#333333"));
  ...
  // left
  drawBranch(treeLeft, windowWidth / 2, windowHeight, 270, 180, 0, -0.5, 255);
  drawBranch(treeLeft, windowWidth / 2, windowHeight, 270, 180, 0, -1.2, 255);
...
function draw() {
  ...
  for (let r = 0; r < rows; r++) {
    for (let c = 0; c < cols; c++) {
      ...
      let rotateAngle = map(sin(frameCount * 0.01), -1, 1, 30, 110);
      ...
      let angle = angleFromMouse + angleFromNoise;
      angles.push(angle);
    }
  }
